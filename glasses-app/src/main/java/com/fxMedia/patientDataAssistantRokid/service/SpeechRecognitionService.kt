package com.fxMedia.patientDataAssistantRokid.service
    ...
    suspend fun transcribe(pcmAudioData: ByteArray): TranscriptionResult {
        return withContext(Dispatchers.IO) {
            try {
                        ...
                        if (candidates != null && candidates.length() > 0) {
                            ...
                            if (parts != null && parts.length() > 0) {
                                ...
                                if (text.contains("Unable to recognize") || text.contains("no sound") || text.isEmpty()) {
                                    return@withContext TranscriptionResult.Error("Please try again")
                                }
                ...
                e.printStackTrace()
                return@withContext TranscriptionResult.Error(e.message ?: "Unknown error")
            }
        }
    }
}
